...
export function createAnimCanvas() {
...
export function clearAnimCanvas() {
  ...
  webglImage.src = '#' // --- Clear previous drawings
  const windData = {
    width,
    height,
    image: webglImage
...
export function animPrepare(width, height, cnvTmp, bnds) {
  ...
  const windData = {
    width,
    height,
    // varMin: this.$store.state.layers.selected.minOrg,
    // varMax: this.$store.state.layers.selected.maxOrg,
    varMin: -3,
    varMax: 3,
    image: webglImage
  }
  webglImage.src = '#' // --- Clear previous drawings
  windData.image = webglImage

  // --- To prepare for redrawing
  cancelAnimationFrame(this.reqAnimID)

  webglImage.src = cnvTmp.toDataURL()
  windData.image = webglImage
